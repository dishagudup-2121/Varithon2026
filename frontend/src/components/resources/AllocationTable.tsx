...
export default function AllocationTable({
  ...
  onAssign: (item: ResourceAllocation) => void;
}) {
  const { t } = useTranslation();
  const navigate = useNavigate();

  const handleTestSimulation = (item: ResourceAllocation) => {
              ...
              <tr>
                <td colSpan={5} className="px-5 py-12 text-center">
                  <div className="flex flex-col items-center justify-center gap-2">
                    <div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#F28C00]/10 text-[#D96F00] mb-2">
                      <Clock size={24} />
                    </div>
                    <p className="font-bold text-[#3D2918] text-lg">
                      No Pending Allocations
                    </p>
                    <p className="text-sm text-[#8B735D] max-w-xs">
                      Integration with the backend API is pending. AI-driven resource recommendations will appear here.
                    </p>
                  </div>
                </td>
              </tr>
      ...
      <div className="space-y-3 p-4 md:hidden">
        ...
        {allocations.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-10 text-center">
            <p className="text-sm font-semibold text-[#8B735D]">
              Awaiting Optimization Engine
            </p>
          </div>
        ) : (
          allocations.map((item) => {
